...
- **Full fetch**: Paginates MOCI API (100 items/page, ~29 pages, ~2800+ codes). For each code: compares API values against existing DB values — INSERT if new, UPDATE only if data actually changed, SKIP if unchanged. Smart skip after 3 consecutive pages with no real changes.
...
- **Stale code deletion**: After a complete fetch (all pages successfully processed), compares all DB codes against all API codes and deletes any that no longer exist in the API. Safety guard: deletion only runs when `fetch_complete=True` — skipped on errors, exceptions, or smart-skip (incomplete data). Prints which codes were deleted.
