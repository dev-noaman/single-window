...
def refresh_session_context(context, page, dp_token, access_id, username, cr_no):
    ...
    Uses page.context.request (Playwright APIRequestContext) to call the API.
    """
...
def save_cert_request(context, dp_token, access_id, username, cr_no, cp_no, cert_type, source_key_id=None):
    ...
    payload = {
        "appRequests": {
            ...
            "counter": False,
            "userId": username,
            "requestType": {"lookupId": cert_type},
            "requestId": None
        },
        "applicantInfo": {
            "identityNo": username,
...
def download_cert_pdf(context, dp_token, access_id, username, request_id, output_filename, ltpa_token=None):
    ...
    if response.ok:
        ...
        if len(pdf_data) < 1000:
            print(f"WARNING: PDF size suspiciously small ({len(pdf_data)} bytes)")
            return False
...
def download_certificate(context, dp_token, access_id, username, cr_no, cp_no, cert_type,
                         output_dir=".", ltpa_token=None, source_key_id=None, page=None):
    """Full workflow: check cache -> save request -> download PDF."""
    ...
    if cached_request_id:
        ...
        request_id = save_cert_request(context, dp_token, access_id, username, cr_no,
                                       cp_no, type_lookup, source_key_id)
        if not request_id:
    ...
    if not success and used_cache:
        print(f"[!] Download failed with cached request - retrying with new request...")
...
def _login_and_get_context(page):
    """
    Helper: wait for login form, fill credentials, wait for dashboard.
    Returns (context, dp_token, access_id, ltpa_token, logged_in).
    Must be called inside a page_action function.
    """
    username = os.getenv('USER_QID', '')
    ...
    for _ in range(30):
        ...
        if "dashboard" in page.url.lower() or dp_token_found:
            logged_in = True
            break

    if not logged_in:
        return context, None, None, None, False
    ...
    return context, dp_token, access_id, ltpa_token, True
...
def search_companies_by_query(query, username, password):
    ...
    Uses Scrapling StealthyFetcher with page_action for login.
    """
    results = []
    seen_crs = set()
    state = {}
    ...
    def do_search(page):
        context, dp_token, access_id, ltpa_token, logged_in = _login_and_get_context(page)
        if not logged_in:
            state['error'] = 'Login timed out. Please check credentials.'
            return
            ...
            for company in companies:
                ...
                if cr and cr not in seen_crs:
                    ...
                    results.append({
                        ...
                        'internal_id': str(
                            company.get('sourceKeyId', company.get('id', company.get('crId', ''))) or ''
                        ) or None
                    })

    try:
        StealthyFetcher.fetch(
            "https://investor.sw.gov.qa/wps/myportal/investors/login/",
            headless=True,
            network_idle=True,
...
def run_company_search(cr_no, username, password, download_cr=False, download_cp=False, output_dir="."):
    ...
    Uses Scrapling StealthyFetcher with page_action for the entire flow.
    """
    print(f"Launching browser to search for CR {cr_no}...")

    def do_login_and_search(page):
        context = page.context

        print("Waiting for login form to load...")
        print("Entering credentials...")

        context2, dp_token, access_id, ltpa_token, logged_in = _login_and_get_context(page)
        context = context2
    ...
    try:
        StealthyFetcher.fetch(
            ...
            headless=True,
            network_idle=True,
            page_action=do_login_and_search,
            timeout=120_000,
        )
    except Exception as e:
        # #region agent log
...
Examples:
  ...
  python auto_search_company.py 76053 --cr --cp -o pdfs  # Download to 'pdfs' folder
    ...
    parser.add_argument('-o', '--output', default=".", help='Output directory for PDF files')
    ...
    run_company_search(args.cr_no, USER_QID, USER_PASS,
                       download_cr=args.cr,
                       download_cp=args.cp,
